...
export default class ThemeSwitch {
  ...
  constructor(root: HTMLElement) {
    ...
    // either would loop or double-write.
    document.addEventListener('theme-change', this.onExternalChange)
  ...
  /** Another live instance changed the preference — mirror it in the radios. */
  private readonly onExternalChange = (event: Event): void => {
    if (event.target === this.root) return // our own dispatch bubbling past
    const detail = (event as CustomEvent<{ preference?: string }>).detail
    if (!detail?.preference) return
    this.preference = resolvePreference(detail.preference)
    this.reflectPreference()
  }

  destroy(): void {
    this.root.removeEventListener('change', this.onChange)
    document.removeEventListener('theme-change', this.onExternalChange)
