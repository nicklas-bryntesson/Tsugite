...
export function generateBaseStylesheet(tables = BASE_TABLES) {
  ...
  return [
    ...
    "}",
    "",
    "/* Writing direction as a sign, for mirrored offsets. :dir() asks for the",
    "   element's computed direction — the nearest dir attribute, dir=auto, or the",
    "   document's default — so every element matches exactly one rule: a closed",
    "   question per element, no base and no override. (An attribute selector asked",
    "   whether the element CARRIED dir, and gave every descendant of an rtl subtree",
    "   the ltr sign.) */",
    ':dir(ltr) { --dir: var(--DIR-LTR); }',
    ':dir(rtl) { --dir: var(--DIR-RTL); }',
    "",
    tierBlocks,
    "",
    gridBlocks,
