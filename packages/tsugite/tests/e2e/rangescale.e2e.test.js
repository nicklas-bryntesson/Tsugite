...
test('a variant takes its colour from the --ui-* seam', async ({ page }) => {
  ...
  // PORT ADAPTATION: our seam maps to @property-registered oklch tokens, so a
  // resolved colour serializes as oklch(...) — the reference assumed rgb.
  for (const ink of inks) expect(ink).toMatch(/^(rgb|oklch|color|lab)/)   // resolved, not a fallback keyword
})
