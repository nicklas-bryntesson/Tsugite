// The recipe is a table read by lib/recipe.ts (ADR-0018). tests/fixtures/textblock.json lists
...
const fixture: Fixture = JSON.parse(readFileSync(new URL("./fixtures/textblock.json", import.meta.url), "utf8"));
  ...
  for (const { name, input, expect: want } of fixture.cases) {
    it(name, async () => {
      ...
      if (want.mode === "render") {
        ...
        expect(html).not.toContain('class="TextBlock"');
      }
    });
  }
});
