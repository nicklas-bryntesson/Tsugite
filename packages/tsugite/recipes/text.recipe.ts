...
// on plain text elements: authored single-run UI text. Both voices speak through the same
// three elements, so there are no absent cells; the size set is the voice's, md by default
...
export const text = {
  name: "Text",
  promise: "One run of words in a quiet voice: body or label, on a plain text element.",
  refuses: [
  ...
  class: "Text",
  element: { values: ["p", "span", "div"], default: "p" },
  parts: {
